...
while i < 5:
    ...
    if user_number<=7 or user_number>=3:
        ...
        array12 = (random.sample(words, user_number))

        string = ''.join(str(x) for x in array12)
        password = (str(user_number)+string)
        se = set(array12)
        if user_number>=5:
            a = set()
            k = a|se
        ...
        print("Your user number must be between 3 and 7 characters.")
...
Dict = {1: 1, 2: 4, 3: 9, 4: 16, 5: 25, 6: 36, 7: 49, 8: 64, 9: 81,
...
print("Max value is: " + str(max(merged_dictionary.values())))
print("Min value is: " + str(min(merged_dictionary.values())))