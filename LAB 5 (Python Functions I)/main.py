...
def rep_with_upper(a):
    str(a)

    x = a[:2].upper() + a[2:].lower()
    return x
...
for x in words:
    ...
    if 7 >= user_number >= 3:
        pas = passGen(user_number)
        print("Your password is: " + pas + "\n")
        big = rep_with_upper(pas)
        print("Uppercase version: " + big + "\n")
        swap = swap_letters(big)
        print("Swapped version: " + swap)
        check = search_letter(swap,"a" + "\n")
        print(check)
        break
